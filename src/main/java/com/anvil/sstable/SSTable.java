...
/**
 * Phase 6: every SSTable now carries a Bloom filter + sparse index, built
 * once at write time and persisted as a ".idx" sidecar next to the ".db"
 * file. get() consults both before touching data: the Bloom filter rules
 * out whole tables that can't contain the key, and the sparse index turns
 * "scan from byte 0" into "seek near the key, then scan a handful of
 * records" -- both directly targeting the ~40x read-latency spread the
 * Phase 5 benchmarks exposed, which came from a plain linear scan.
 *
 * Tables written before Phase 6 (no sidecar on disk) self-heal: the first
 * time one is opened, its Bloom filter and index are rebuilt from a single
 * scan of the file and the sidecar is written out for next time.
 */
public class SSTable {
    ...
    // ---------------------------------------------------------------------
    // Writing (used by both flush and compaction)
    // ---------------------------------------------------------------------
    ...
    public byte[] get(String targetKey) throws IOException {
        if (bloomFilter != null && !bloomFilter.mightContain(targetKey)) {
            return null; // definitely not in this table -- no I/O needed
        }

        long startOffset = 0;
        if (sparseIndex != null) {
            startOffset = sparseIndex.startOffsetFor(targetKey);
            if (startOffset < 0) return null; // smaller than every key here
        }
        ...
        try (RandomAccessFile raf = new RandomAccessFile(file, "r")) {
            ...
            while (true) {
                ...
                if (cmp == 0) {
                    ...
                    return null; // sorted file: we've gone past where it would be
                } else {
                    if (!isTombstone) {
                        int valLen = raf.readInt();
    ...
    // ---------------------------------------------------------------------
    // Sidecar (Bloom filter + sparse index) persistence and self-healing
    // ---------------------------------------------------------------------
    ...
    private void loadOrBuildSidecar() {
        ...
        if (sidecar.exists()) {
            try (DataInputStream in = new DataInputStream(
                ...
                // corrupt or partial sidecar -- fall through and rebuild it below
            }
        }
        rebuildSidecarByScanning(sidecar);
    }

    /** One-time self-heal for tables that predate Phase 6 or whose sidecar was lost. */
    ...
    private static void writeSidecarQuietly(File sidecar, BloomFilter bloom, SparseIndex index) {
        try (DataOutputStream out = new DataOutputStream(
            ...
            // the sidecar is a cache, not the source of truth -- safe to skip on failure
        }
    }

    // ---------------------------------------------------------------------
    // Nested: Bloom filter (no separate file -- kept inside SSTable itself)
    // ---------------------------------------------------------------------

    /**
     * Classic Bloom filter, one per SSTable. Answers "is this key definitely
     * absent from this table?" without touching the file. False positives
     * are possible; false negatives are not, so correctness never depends
     * on it -- it can only save work.
     */
    private static final class BloomFilter {
        ...
        // 64-bit mix (MurmurHash3 finalizer style), seeded two different ways.
    ...
    // ---------------------------------------------------------------------
    // Nested: sparse index (no separate file -- kept inside SSTable itself)
    // ---------------------------------------------------------------------

    /**
     * Sparse (key -> byte offset) index, sampled every `interval` records.
     * Since records are written in sorted key order, floorEntry(target)
     * gives the furthest safe seek point before falling back to scanning a
     * handful of records by hand. Record 0 is always indexed, so a miss on
     * floorEntry proves targetKey is smaller than everything in the table.
     */
